...
<?php
	$rs = mysqli_query($con, "select * from question order by id");
?>

	  <div class="container">
	  
	  <h2>FAQs</h2>
	  <h3><a href="add-faq.php" class="btn" title="Add FAQ"><i class="fa fa-plus"></i></a></h3>
...
?>
	<tr>
		...
		<td><?=$row[1]?></td>
		<td><?=$row[2]?></td>
		<td><a href="delete-question.php?id=<?=$row[0]?>" class="btn" title="Delete" onclick="return confirm('Delete question?')"><i class="fa fa-trash-o"></i></a></td>
		<td><a href="edit-question.php?id=<?=$row[0]?>" class="btn" title="Edit"><i class="fa fa-edit"></i></a></td>
	</tr>
<?php
	}
?>	
