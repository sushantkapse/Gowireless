...
<?php
	$rs = mysqli_query($con, "select bill_id, bill_date, plan_name, cost, emp_id, status from plans, bill where bill.plan_id = plans.plan_id order by bill_id");
?>
...
<?php
	while($row = mysqli_fetch_row($rs)){
		$rs1 = mysqli_query($con, "select emp_name from employee where emp_id=".$row[4]);
...
?>
	<tr>
		...
		<td><?=$engineer?></td>
		<td><?=$row[5]?></td>
		<td><a href="assign-engineer.php?bid=<?=$row[0]?>" class="btn">Assign Engineer</a></td>
		<td><a href="close-bill.php?bid=<?=$row[0]?>" class="btn">Close</a></td>
		<td><a href="view-bill.php?bid=<?=$row[0]?>" class="btn">View Bill</a></td>
	</tr>
<?php
	}
?>	
